 ...
 */
/*
 在 Mach-O 中，Header 的结构是固定的
 file offset 固定为 0
 长度为 32 bytes（对于 64 bit 架构）
 */
struct mach_header_64 {
	uint32_t	magic;		/* mach magic number identifier */  标识是32bit/64bit
	...
	// dyld 标识 flags 是杂项，通常它包含的信息用于为动态链接器服务
	uint32_t	flags;		/* flags */
 ...
 */
/*
 Load Commands 可以被看作是一个 command 列表，紧贴着 Header，所以它的 file offset 是固定的：0x20。一共有哪些 load commands 呢？
 Load commands 由内核定义，不同版本的 command 数量不同，一共定义了 50+ load commands，它们的 type 是以LC_为前缀常量，譬如 LC_SEGMENT、LC_SYMTAB 等。
 ...
 */
// LC_SEGMENT_64指令对应的结构
/*
 从这个结构体我们能看出什么？它描述了文件映射的两大问题：从哪里来（fileoff、filesize）、到哪里去（vmaddr、vmsize）；
 它还告诉了内核该区域的名字（segname，即 segment name），以及该区域包含了几个 section（nsects），以及该区域的保护级别（initprot、maxprot）。
 
 每一个 segment 的 VP (Virtual Page) 都根据 initprot 进行初始化，initprot 指定了如何通过读/写/执行位初始化页面的保护级别；
 segment 的保护设置可以动态改变，但是不能超过 maxprot 中指定的值（在 iOS 中，+x 和+w 是互斥的）；
 initprot、maxprot 的值均用八进制表示（4=r，2=w，1=x）
